/**
 * Cubism 2 Live2D 管理器
 * 专门处理 Live2D Cubism 2 格式模型的加载和渲染
 */
...
export interface CubismModelConfig {
  modelPath: string;
  width: number;
  height: number;
  scale: number;
...
export class CubismLive2DManager {
   ...
   * 检查 Live2D 运行时是否可用
   */
  private checkLive2DRuntime(): boolean {
    if (typeof window !== 'undefined' && window.Live2D) {
      console.log('Live2D Cubism 2 runtime found');
      return true;
    }
    console.error('Live2D Cubism 2 runtime not found. Please ensure live2d.min.js is loaded.');
    return false;
  }
   ...
   * 等待 Live2D 运行时加载
   */
  private async waitForLive2DRuntime(): Promise<boolean> {
   ...
   * 初始化管理器
   */
  async initialize(container: HTMLElement, config: CubismModelConfig): Promise<void> {
    ...
    try {
      ...
      console.log('Initializing CubismLive2DManager...');

      // 等待 Live2D 运行时加载
      const runtimeReady = await this.waitForLive2DRuntime();
      if (!runtimeReady) {
      ...
      console.log('CubismLive2DManager initialized successfully');
    } catch (error) {
      this.isLoading = false;
      console.error('Failed to initialize CubismLive2DManager:', error);
      // 创建占位符
   ...
   * 加载 Live2D 模型
   */
  private async loadModel(config: CubismModelConfig): Promise<void> {
   ...
   * 设置模型属性
   */
  private setupModel(config: CubismModelConfig): void {
  ...
  destroy(): void {
    ...
    console.log('CubismLive2DManager destroyed');
  }
...
export default CubismLive2DManager;
